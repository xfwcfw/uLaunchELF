...
static void Command_List( void )
{
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event.
			...
			printXY((u8 *)LNG(Start_StartStop_Slideshow), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(L1R1_Slideshow_Timer), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(L2R2_Slideshow_Transition), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(LeftRight_Pad_PrevNext_Picture), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(UpDown_Pad_Rotate_Picture), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(Left_Joystick_Panorama), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			printXY((u8 *)LNG(Right_Joystick_Vertical_Zoom), x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
			if (swapKeys)
				sprintf(tmp, "ÿ1: %s", LNG(FullScreen_Mode));
			else
				sprintf(tmp, "ÿ0: %s", LNG(FullScreen_Mode));
			printXY((u8 *)tmp, x, y, setting->color[3], TRUE, 0);
			y+=FONT_HEIGHT;
...
frame:
					...
					Len = printXY((u8 *)tmp, 0, 0, 0, FALSE, 0);
					if(Len>72 && top+i==jpg_browser_sel){
						...
						Len = printXY((u8 *)tmp1, 0, 0, 0, FALSE, 0);
						printXY((u8 *)tmp1, x+72/2-Len/2, (y+58), color, TRUE, 0);
					}else if(Len>72){
						tmp[7]='.'; tmp[8]='.'; tmp[9]='.'; tmp[10]='\0';
						printXY((u8 *)tmp, x-4, (y+58), color, TRUE, 0);
					}else
						printXY((u8 *)tmp, x+72/2-Len/2, (y+58), color, TRUE, 0);
