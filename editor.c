	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event.
			...
			for(i=0,y=mSprite_Y1+FONT_HEIGHT/2; i<NUM_MENU; i++){
				...
				printXY((u8 *)tmp, mSprite_X1+2*FONT_WIDTH, y, color, TRUE, 0);
				y+=FONT_HEIGHT;
			}
			...
			printXY((u8 *)tmp, x, y, setting->color[2], TRUE, 0);
	...
	while(1){
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event.
			...
			for(i=0,y=wSprite_Y1+FONT_HEIGHT/2; i<10; i++){
				...
				if(!Window[i][OPENED])
					printXY((u8 *)LNG(Free_Window), wSprite_X1+2*FONT_WIDTH, y, color, TRUE, 0);
				else if(Window[i][CREATED])
					printXY((u8 *)LNG(Window_Not_Yet_Saved), wSprite_X1+2*FONT_WIDTH, y, color, TRUE, 0);
				else if(Window[i][OPENED])				
					printXY((u8 *)Path[i], wSprite_X1+2*FONT_WIDTH, y, color, TRUE, 0);
		...
		if(event||post_event){ //NB: We need to update two frame buffers per event.
			...
			if(KeyBoard_Active){ //Display Virtual KeyBoard Section.
				...
				if(Mark[MARK_ON])
					...
					color=setting->color[3];
				printXY((u8 *)LNG(MARK), KEY_X+2+4-120, KEY_Y+12,
					color, TRUE, ((KEY_X-48)-SCREEN_MARGIN-3*FONT_WIDTH));
				printXY((u8 *)LNG(LINE_UP), KEY_X+2+4-120+10*FONT_WIDTH, KEY_Y+12,
					setting->color[3], TRUE, ((KEY_X+32)-(KEY_X-48)-3*FONT_WIDTH));
				if(Mark[MARK_COPY])
					color=setting->color[2];
				else
					color=setting->color[3];
				printXY((u8 *)LNG(COPY), KEY_X+2+4-120, KEY_Y+12+FONT_HEIGHT+2,
					color, TRUE, ((KEY_X-48)-SCREEN_MARGIN-3*FONT_WIDTH));
				printXY((u8 *)LNG(LINE_DOWN), KEY_X+2+4-120+10*FONT_WIDTH, KEY_Y+12+FONT_HEIGHT+2,
					setting->color[3], TRUE, ((KEY_X+32)-(KEY_X-48)-3*FONT_WIDTH));
				if(Mark[MARK_CUT])
					color=setting->color[2];
				else
					color=setting->color[3];
				printXY((u8 *)LNG(CUT), KEY_X+2+4-120, KEY_Y+12+FONT_HEIGHT*2+4,
					color, TRUE, ((KEY_X-48)-SCREEN_MARGIN-3*FONT_WIDTH));
				printXY((u8 *)LNG(PAGE_UP), KEY_X+2+4-120+10*FONT_WIDTH, KEY_Y+12+FONT_HEIGHT*2+4,
					setting->color[3], TRUE, ((KEY_X+32)-(KEY_X-48)-3*FONT_WIDTH));
				printXY((u8 *)LNG(PASTE), KEY_X+2+4-120, KEY_Y+12+FONT_HEIGHT*3+6,
					setting->color[3], TRUE, ((KEY_X-48)-SCREEN_MARGIN-3*FONT_WIDTH));
				printXY((u8 *)LNG(PAGE_DOWN), KEY_X+2+4-120+10*FONT_WIDTH, KEY_Y+12+FONT_HEIGHT*3+6,
					setting->color[3], TRUE, ((KEY_X+32)-(KEY_X-48)-3*FONT_WIDTH));
				printXY((u8 *)LNG(HOME), KEY_X+2+4-120, KEY_Y+12+FONT_HEIGHT*4+8,
					setting->color[3], TRUE, ((KEY_X-48)-SCREEN_MARGIN-3*FONT_WIDTH));
				printXY((u8 *)LNG(END), KEY_X+2+4-120+10*FONT_WIDTH, KEY_Y+12+FONT_HEIGHT*4+8,
					setting->color[3], TRUE, ((KEY_X+32)-(KEY_X-48)-3*FONT_WIDTH));
				...
				if(Editor_Insert)
					...
					color=setting->color[3];
				printXY((u8 *)LNG(INSERT), KEY_X+2+4+392, KEY_Y+12,
				...
				printXY((u8 *)tmp, KEY_X+2+4 + 392, KEY_Y+12+FONT_HEIGHT+2,
					setting->color[3], TRUE, ((SCREEN_WIDTH-SCREEN_MARGIN)-(KEY_X+KEY_W+32)-3*FONT_WIDTH));
				printXY((u8 *)LNG(TAB), KEY_X+2+4 + 392, KEY_Y+12+FONT_HEIGHT*2+4,
					setting->color[3], TRUE, ((SCREEN_WIDTH-SCREEN_MARGIN)-(KEY_X+KEY_W+32)-3*FONT_WIDTH));
				printXY((u8 *)LNG(SPACE), KEY_X+2+4 + 392, KEY_Y+12+FONT_HEIGHT*3+6,
					setting->color[3], TRUE, ((SCREEN_WIDTH-SCREEN_MARGIN)-(KEY_X+KEY_W+32)-3*FONT_WIDTH));
				printXY((u8 *)LNG(KB_RETURN), KEY_X+2+4 + 392, KEY_Y+12+FONT_HEIGHT*4+8,
					setting->color[3], TRUE, ((SCREEN_WIDTH-SCREEN_MARGIN)-(KEY_X+KEY_W+32)-3*FONT_WIDTH));
